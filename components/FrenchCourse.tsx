...
const COURSE_META: Record<string, { achieves: string[]; emoji: string }> = {
  'french-real-life-communication': {
    emoji: '🗣️',
    achieves: [
      'Hold natural conversations in French',
      'Understand native French speakers',
      'Express yourself in daily situations',
      'Build vocabulary for everyday life',
    ],
  },
  'french-travel-cultural': {
    emoji: '✈️',
    achieves: [
      'Travel confidently through France & francophone countries',
      'Handle restaurants, hotels, and transport',
      'Understand French cultural etiquette',
      'Connect meaningfully with locals',
    ],
  },
  'french-academic': {
    emoji: '🎓',
    achieves: [
      'Write academic essays and reports in French',
      'Understand university lectures and seminars',
      'Use formal academic vocabulary',
      'Communicate in professional academic settings',
    ],
  },
  'french-conversational-mastery': {
    emoji: '💬',
    achieves: [
      'Speak French spontaneously and fluently',
      'Move beyond textbook to real conversation',
      'Understand regional accents and slang',
      'Think naturally in French',
    ],
  },
};
...
export default function FrenchCourse() {
  ...
  useEffect(() => {
    fetch('/api/courses?category=french')
      .then(r => r.json())
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (saved) { try { const u = JSON.parse(saved); setUser(u); checkAllEnrollments(u.id); } catch {} }
      }
      setLoading(false);
    };
    init();
  ...
  const checkAllEnrollments = async (userId: string) => {
    ...
    results.forEach(r => { if (r.status === 'fulfilled') map[r.value.courseId] = { enrolled: r.value.enrolled, status: r.value.status }; });
    setEnrollments(map);
  };
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 600; i++) positions.push((Math.random() - 0.5) * 200, (Math.random() - 0.5) * 200, (Math.random() - 0.5) * 200);
  ...
  const handleEnroll = (course: CourseData) => {
    if (!user) { toast.info('Please login to enroll in this course'); router.push('/login?redirect=/FrenchCourse'); return; }
    ...
    <>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-14">
            ...
            <p className="text-blue-300 text-lg max-w-2xl mx-auto">
              4 targeted courses built for real communication — from everyday French to academic fluency. Find your level and start speaking with confidence.
            </p>
            <div className="flex flex-wrap justify-center gap-6 mt-8">
              {[{ icon: Users, value: '8K+', label: 'Students' }, { icon: Globe, value: '30+', label: 'Countries' }, { icon: Infinity, value: 'Lifetime', label: 'Access' }].map(({ icon: Icon, value, label }) => (
          ...
          {courses.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-14">
              {courses.map((course) => {
                ...
                return (
                  <div key={course.courseId} className="rounded-2xl overflow-hidden flex flex-col transition-transform hover:scale-[1.01]"
                    ...
                    <div className="p-6 flex flex-col flex-1">
                      <div className="flex items-start justify-between gap-3 mb-4">
                        ...
                        {isEnrolled && <span className="flex-shrink-0 flex items-center gap-1 text-green-400 text-xs font-bold"><CheckCircle className="w-4 h-4" />Enrolled</span>}
                        {isPending && !isEnrolled && <span className="flex-shrink-0 flex items-center gap-1 text-yellow-400 text-xs font-bold"><Clock className="w-4 h-4" />Pending</span>}
                      </div>
                      ...
                      {meta?.achieves && (
                        <ul className="space-y-1.5 mb-5 flex-1">
                          {meta.achieves.map(item => (
                            <li key={item} className="flex items-start gap-2 text-blue-100 text-sm">
                              <CheckCircle className="w-3.5 h-3.5 text-green-400 mt-0.5 flex-shrink-0" />{item}
                            </li>
                          ))}
                        </ul>
                      )}
                          ...
                          <div className="flex items-baseline gap-2">
                            ...
                            {course.discount > 0 && (
                              <>
                                ...
                                <span className="px-1.5 py-0.5 rounded text-xs font-bold text-green-300" style={{ background: 'rgba(34,197,94,0.15)', border: '1px solid rgba(34,197,94,0.3)' }}>{course.discount}% OFF</span>
                              </>
                            )}
                          </div>
                          <button
                            ...
                            style={{ background: isEnrolled ? 'rgba(34,197,94,0.25)' : isPending ? 'rgba(234,179,8,0.25)' : 'linear-gradient(135deg,#2563eb,#1d4ed8)', boxShadow: isEnrolled || isPending ? 'none' : '0 0 20px rgba(37,99,235,0.4)' }}
                          >
      ...
      {activePayment && (
        <PaymentModal
          ...
          onSuccess={() => setEnrollments(prev => ({ ...prev, [activePayment.courseId]: { enrolled: false, status: 'PENDING' } }))}
        />
      )}
    </>
  );
