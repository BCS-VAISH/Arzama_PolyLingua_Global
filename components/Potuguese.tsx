...
const COURSE_META: Record<string, { achieves: string[]; emoji: string }> = {
  'portuguese-real-life-communication': {
    emoji: '🗣️',
    achieves: [
      'Speak Portuguese in everyday conversations',
      'Understand native speakers naturally',
      'Express yourself clearly in daily life',
      'Build practical vocabulary fast',
    ],
  },
  'portuguese-travel-cultural': {
    emoji: '✈️',
    achieves: [
      'Explore Brazil & Portugal like a local',
      'Handle travel situations with confidence',
      'Understand cultural customs and etiquette',
      'Make genuine connections with locals',
    ],
  },
  'portuguese-business-professional': {
    emoji: '💼',
    achieves: [
      'Write professional emails and reports',
      'Communicate in meetings and negotiations',
      'Build your professional network in Portuguese',
      'Present ideas clearly and confidently',
    ],
  },
  'portuguese-conversational-mastery': {
    emoji: '💬',
    achieves: [
      'Speak fluently and spontaneously',
      'Think naturally in Portuguese',
      'Understand regional accents and expressions',
...
export default function Potu() {
  ...
  useEffect(() => {
    fetch('/api/courses?category=portuguese')
      .then(r => r.json())
  ...
  const handleEnroll = (course: CourseData) => {
    if (!user) { toast.info('Please login to enroll in this course'); router.push('/login?redirect=/PortugueseCourse'); return; }
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-14">
            ...
            <p className="text-blue-300 text-lg max-w-2xl mx-auto">
              4 practical courses designed for real fluency — from everyday communication to professional mastery. Start your Portuguese journey today.
            </p>
            <div className="flex flex-wrap justify-center gap-6 mt-8">
              {[{ icon: Users, value: '6K+', label: 'Students' }, { icon: Globe, value: '25+', label: 'Countries' }, { icon: Infinity, value: 'Lifetime', label: 'Access' }].map(({ icon: Icon, value, label }) => (
